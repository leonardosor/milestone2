    ...
    try:
        with engine.connect() as conn:
            ...
            for col in columns:
                ...
                print(f"  {col_name:<25} {data_type}{length_str:<20} {nullable_str:<10}{default_str}")
...
def interactive_menu(engine):
    ...
    while True:
        ...
        if current_df is not None:
            print(f"📊 Current DataFrame: {current_schema}.{current_table} ({current_df.shape[0]} rows × {current_df.shape[1]} columns)")
        ...
        if choice == '1':
            ...
            if schemas:
                ...
                if schema_name in schemas:
                    ...
                    if tables:
                        ...
                        table_name = input("Enter table name: ").strip()
                        if table_name in tables:
                            limit_input = input("Enter row limit (press Enter for all rows): ").strip()
                            limit = int(limit_input) if limit_input.isdigit() else None
                            
                            # Load the DataFrame
                            df = load_table_as_dataframe(engine, schema_name, table_name, limit)
                            if df is not None:
                                current_df = df
            ...
            # Filter DataFrame
            if current_df is not None:
                filtered_df = filter_dataframe(current_df, current_schema, current_table)
                if filtered_df is not None:
                    ...
                    replace = input("Replace current DataFrame with filtered version? (y/n): ").strip().lower()
                    if replace == 'y':
...
def filter_dataframe(df, schema_name, table_name):
    ...
    try:
        ...
        print(f"✅ Filter applied: {len(filtered_df)} rows match '{column}' = '{filter_value}'")
