...
def geocode_with_uszipcode():
    ...
    try:
        ...
        for i in range(0, len(valid_coords), batch_size):
            ...
            logger.info(f"Processed {processed:,}/{len(valid_coords):,} coordinates ({rate:.0f}/sec)")
        ...
        logger.info(f"Processing rate: {len(results)/total_time:.0f} coordinates/second")
...
def geocode_with_reverse_geocoder():
    ...
    try:
        ...
        logger.info(f"Processing {len(valid_coords):,} coordinates with reverse-geocoder...")
        ...
        logger.info(f"Successful: {successful:,}")
        logger.info(f"Processing rate: {len(processed_results)/total_time:.0f} coordinates/second")
...
def create_zipcode_lookup_table():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not initialize USZipcode SearchEngine for lookup: {e}")
            return 0
        ...
        # Add primary key
        cur.execute("ALTER TABLE lat_lon_zipcode_lookup ADD COLUMN id SERIAL PRIMARY KEY")
...
def fast_geocode_coordinates(table_name="location_data"):
    ...
    try:
        ...
        # Summary
        total_time = time.time() - start_time
        successful = sum(1 for r in processed_results if r[2] or r[3])  # County or state available
...
def main():
    ...
    if success:
        total_time = time.time() - start_time
        logger.info(f"Fast geocoding completed successfully in {total_time:.1f} seconds")
        return True
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fast offline geocoding using reverse-geocoder")
    parser.add_argument(
        "--table-name", 
        type=str, 
