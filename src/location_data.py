...
def _ensure_shapefile(zip_filename: str, subdir: str, data_dir: Path):
    ...
    if not zip_path.exists():
        raise FileNotFoundError(f"Missing {zip_filename}. Run with --download-data to fetch.")
    extract_dir = data_dir / subdir
    shp_exists = any(p.suffix.lower()=='.shp' for p in extract_dir.glob('*.shp')) if extract_dir.exists() else False
    if not shp_exists:
...
def prepare_datasets(data_dir: Path, force_download: bool = False) -> Tuple[Path, Path, Path]:
    ...
    if force_download:
        ...
        # Download if missing
        for folder, fname in (("ZCTA520", ZCTA_ZIP), ("COUNTY", COUNTY_ZIP), ("STATE", STATE_ZIP)):
...
def load_geodata(zcta_dir: Path, county_dir: Path, state_dir: Path):
    ...
    county_gdf = gpd.read_file(first_shp(county_dir))[["NAME", "STATEFP", "GEOID", "geometry"]]
    state_gdf = gpd.read_file(first_shp(state_dir))[["NAME", "STUSPS", "STATEFP", "geometry"]]

    # Ensure all are in WGS84 (EPSG:4326)
    for g in (zcta_gdf, county_gdf, state_gdf):
        if g.crs is None:
            logger.warning("A layer has no CRS; assuming EPSG:4269 (NAD83) -> converting to EPSG:4326")
...
def save_tiger_to_db(zcta_gdf, county_gdf, state_gdf, table_name="census_geodata"):
    ...
    try:
        ...
        logger.info(f"Successfully saved {total_records:,} TIGER records to {table_name}")
        logger.info(f"ZCTA: {len(zcta_records):,}, County: {len(county_records):,}, State: {len(state_records):,}")
...
def spatial_join_points(points_df, zcta_gdf, county_gdf, state_gdf):
    ...
    pts_state = gpd.sjoin(gdf_pts, state_gdf[["NAME", "STATEFP", "geometry"]], predicate="within", how="left", rsuffix="_state")
    pts_state.rename(columns={"NAME": "state", "STATEFP": "state_fips"}, inplace=True)
    pts_county = gpd.sjoin(pts_state, county_gdf[["NAME", "STATEFP", "geometry"]], predicate="within", how="left", rsuffix="_county")
    pts_county.rename(columns={"NAME": "county"}, inplace=True)
    pts_zcta = gpd.sjoin(pts_county, zcta_gdf, predicate="within", how="left", rsuffix="_zcta")
    pts_zcta.rename(columns={"ZCTA5CE20": "zip"}, inplace=True)

    result = pts_zcta[["latitude", "longitude", "zip", "county", "state", "state_fips"]].copy()
...
def geocode_coordinates_to_location_data(table_name="location_data", data_dir: Optional[str] = None, force_download: bool = False):
    """Geocode using TIGER/Line shapefiles (ZCTA, County, State)."""
    try:
        data_path = _get_data_dir(data_dir)
        zcta_dir, county_dir, state_dir = prepare_datasets(data_path, force_download=force_download)
        ...
        if not save_tiger_to_db(zcta_gdf, county_gdf, state_gdf, table_name="census_geodata"):
            logger.warning("Failed to save TIGER data to database, but continuing with geocoding...")
        ...
        logger.info(f"Spatial joins complete in {elapsed:.1f}s. ZIP coverage: {(enriched['zip']!='').mean()*100:.1f}%")
        ...
        logger.info(f"ZIP codes populated: {(enriched['zip']!='').sum():,} ({(enriched['zip']!='').mean()*100:.1f}%)")
        conn.close()
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Offline TIGER/Line geocoder (ZIP/County/State)")
    parser.add_argument("--table-name", default="location_data", help="Destination table name")
    parser.add_argument("--data-dir", default=None, help="Directory to store/download TIGER shapefiles")
    parser.add_argument("--download-data", action="store_true", help="Force download TIGER datasets")
    parser.add_argument("--test-only", action="store_true", help="Only test DB connectivity")
