...
class FBIDataETL:
    ...
    def fetch_arrest_data(self, ori: str, offense: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(
                f"Unexpected error fetching arrest data for {ori}/{offense}: {e}"
            )
            return None
        ...
        try:
            ...
            if "latitude" in df_clean.columns:
                df_clean["latitude"] = pd.to_numeric(
                    df_clean["latitude"], errors="coerce"
                )
    ...
    def run_arrests_etl(
        self, limit_agencies: int = None, limit_offenses: List[str] = None
    ):
        ...
        try:
            ...
            logger.info(
                f"Fetching arrest data for {len(agencies_df)} agencies and {len(offenses_to_fetch)} offense types"
            )
            ...
            logger.info(
                f"✅ FBI arrests ETL process completed. Total records: {total_records}"
            )
    ...
    def run_full_etl(
        ...
        try:
            ...
            if agencies:
                self.run_arrests_etl(
                    limit_agencies=limit_agencies, limit_offenses=limit_offenses
                )
...
def main():
    ...
    parser.add_argument(
        "--offenses", nargs="+", help="Specific offense types to fetch (default: all)"
    )
    ...
    try:
        ...
        if args.mode == "agencies":
            ...
            etl.run_arrests_etl(
                limit_agencies=args.limit_agencies, limit_offenses=args.offenses
            )
        else:  # full
            etl.run_full_etl(
                limit_agencies=args.limit_agencies, limit_offenses=args.offenses
