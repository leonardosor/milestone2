...
class AsyncUrbanDataETL:
        ...
        try:
            with self.engine.connect() as conn:
                if self.drop_existing_tables:
                    ...
                    conn.execute(text("DROP TABLE IF EXISTS urban_institute_data CASCADE;"))
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"Successfully fetched data from {endpoint} for year {year}")
                        return data
                    else:
    ...
    async def insert_data_batch(self, data_batch):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                df.to_sql('urban_institute_data', conn, if_exists='append', index=False, method='multi')
                conn.commit()
    ...
    async def run_etl_async(self, begin_year=None, end_year=None, endpoints=None):
        ...
        try:
            ...
            logger.info("Created tables: urban_institute_data, urban_data_expanded, urban_data_directory")
            logger.info("=" * 80)
...
class UrbanDataSplitter:
    ...
    def analyze_json_structure(self, begin_year=None, end_year=None) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            if df.empty:
                logger.warning("No data found in urban_data table for the specified criteria")
                return {}
            ...
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    logger.warning(f"Could not parse data_json for endpoint {endpoint}: {e}")
                    continue
    ...
    def get_all_unique_keys(self, begin_year=None, end_year=None, exclude_directory=False, directory_only=False) -> Set[str]:
        ...
        try:
            ...
            logger.info(f"Found {len(keys)} unique keys from data_json column{filter_desc}: {', '.join(sorted(keys))}")
            return keys
    ...
    def create_expanded_table(self, table_name="urban_data_expanded", begin_year=None, end_year=None, exclude_directory=False):
        ...
        try:
            ...
            unique_keys = self.get_all_unique_keys(begin_year, end_year, exclude_directory=exclude_directory)
            ...
            with self.engine.connect() as conn:
                ...
                standard_column_names = {'id', 'data_source', 'endpoint', 'year', 'fetched_at', 'created_at'}
            ...
            logger.info(f"Successfully created {table_name} with {len(json_columns)} JSON-derived columns")

        except Exception as e:
            logger.error(f"Failed to create expanded table: {e}")
            raise

    def create_directory_table(self, table_name="urban_data_directory", begin_year=None, end_year=None):
        ...
        try:
            ...
            unique_keys = self.get_all_unique_keys(begin_year, end_year, directory_only=True)
            ...
            logger.info(f"Successfully created directory table {table_name} with {len(json_columns)} JSON-derived columns")

        except Exception as e:
            logger.error(f"Failed to create directory table: {e}")
            raise

    def populate_expanded_table(self, table_name="urban_data_expanded", batch_size=1000, begin_year=None, end_year=None, exclude_directory=False):
        ...
        if begin_year is not None and end_year is not None:
            ...
            logger.info(f"Populating expanded table: {table_name} for years {begin_year}-{end_year}")
        else:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                while processed < total_records:
                    logger.info(f"Processing batch: {processed + 1} to {min(processed + batch_size, total_records)}")
                    ...
                    if full_filter:
                        query = base_query + full_filter + f" ORDER BY id LIMIT {batch_size} OFFSET {processed}"
                    else:
                        query = base_query + f" ORDER BY id LIMIT {batch_size} OFFSET {processed}"
                        ...
                        try:
                            ...
                            if isinstance(json_data, dict):
                                for key in unique_keys:
                                    clean_key = self._sanitize_column_name(key)
                                    record[clean_key] = str(json_data.get(key, '')) if json_data.get(key) is not None else ''
                            else:
                            ...
                            logger.warning(f"Could not parse data_json JSON for record {row['id']}: {e}")
                    ...
                    processed += len(df)
                    logger.info(f"Processed {processed}/{total_records} records ({processed/total_records*100:.1f}%)")
                
                logger.info(f"Successfully populated {table_name} with {processed} records")

        except Exception as e:
            logger.error(f"Failed to populate expanded table: {e}")
            raise

    def populate_directory_table(self, table_name="urban_data_directory", batch_size=1000, begin_year=None, end_year=None):
        ...
        if begin_year is not None and end_year is not None:
            year_filter = f" AND year BETWEEN {begin_year} AND {end_year}"
            logger.info(f"Populating directory table: {table_name} for years {begin_year}-{end_year}")
        else:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                # Process in batches
                while processed < total_records:
                    logger.info(f"Processing directory batch: {processed + 1} to {min(processed + batch_size, total_records)}")
                    ...
                    query = base_query + endpoint_filter + year_filter + f" ORDER BY id LIMIT {batch_size} OFFSET {processed}"
                    result = conn.execute(text(query))
                    df = pd.DataFrame(result.fetchall(), columns=result.keys())
                    ...
                    processed += len(df)
                    logger.info(f"Processed {processed}/{total_records} directory records ({processed/total_records*100:.1f}%)")
                
                logger.info(f"Successfully populated directory table {table_name} with {processed} records")
        ...
        try:
            with self.engine.connect() as conn:
                ...
                for i, record in enumerate(sample_data):
                    ...
                    for j, (col_name, _) in enumerate(columns[:5]):  # Show first 5 columns
    ...
    def create_analysis_views(self, table_name="urban_data_expanded", directory_table_name="urban_data_directory"):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                dir_columns_result = conn.execute(text(f"""
                    SELECT column_name 
                    FROM information_schema.columns 
                    WHERE table_name = '{directory_table_name}' 
                    ORDER BY ordinal_position
                """))
                dir_available_columns = {row[0] for row in dir_columns_result.fetchall()}
                
                # Get actual columns from the expanded table
                exp_columns_result = conn.execute(text(f"""
                    SELECT column_name 
                    FROM information_schema.columns 
                    WHERE table_name = '{table_name}' 
                    ORDER BY ordinal_position
                """))
                exp_available_columns = {row[0] for row in exp_columns_result.fetchall()}
                
                logger.info(f"Available columns in {directory_table_name}: {len(dir_available_columns)} columns")
                logger.info(f"Available columns in {table_name}: {len(exp_available_columns)} columns")
                ...
                def filter_existing_columns(column_map, available_columns):
                    return [col for col in column_map.values() if col in available_columns]
                
                # Create schools directory view from directory table
                directory_cols = filter_existing_columns(directory_columns, dir_available_columns)
                if directory_cols:
                    schools_view = f"""
                    ...
                    logger.info(f"Created schools_directory_view with {len(directory_cols)} columns from {directory_table_name}")
                else:
                    logger.warning("Skipped schools_directory_view - no matching columns found")
                
                # Create enrollment view from expanded table
                enrollment_cols = filter_existing_columns(enrollment_columns, exp_available_columns)
                if enrollment_cols:
                    enrollment_view = f"""
                    ...
                    logger.info(f"Created enrollment_view with {len(enrollment_cols)} columns from {table_name}")
                else:
                    logger.warning("Skipped enrollment_view - no matching columns found")
                ...
                if test_cols:
                    ...
                    logger.info(f"Created test_participation_view with {len(test_cols)} columns from {table_name}")
                else:
                    logger.warning("Skipped test_participation_view - no matching columns found")
    ...
    def run_split_process(self, table_name="urban_data_expanded", begin_year=None, end_year=None):
        ...
        try:
            ...
            self.create_expanded_table(table_name, begin_year, end_year, exclude_directory=True)
            
            # Step 4: Create directory table for directory data
            directory_table_name = f"{table_name.replace('_expanded', '')}_directory"
            logger.info(f"Step 4/8: Creating directory table: {directory_table_name}...")
            self.create_directory_table(directory_table_name, begin_year, end_year)
            
            # Step 5: Populate expanded table (non-directory data)
            logger.info("Step 5/8: Populating expanded table with non-directory data...")
            self.populate_expanded_table(table_name, begin_year=begin_year, end_year=end_year, exclude_directory=True)
            
            # Step 6: Populate directory table
            logger.info(f"Step 6/8: Populating directory table: {directory_table_name}...")
            self.populate_directory_table(directory_table_name, begin_year=begin_year, end_year=end_year)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Urban Institute ETL and Data Splitting")
    parser.add_argument("--config", type=str, default="config.json", help="Configuration file path")
    parser.add_argument("--mode", type=str, choices=["etl", "split"], default="etl", 
                       help="Mode: 'etl' for full ETL process, 'split' for splitting only")
    parser.add_argument("--table-name", type=str, default="urban_data_expanded", help="Name for expanded table")
    ...
    if (args.begin_year is not None and args.end_year is None) or (args.begin_year is None and args.end_year is not None):
        logger.error("Both --begin-year and --end-year must be specified together, or neither")
        sys.exit(1)
    
    if args.begin_year is not None and args.end_year is not None and args.begin_year > args.end_year:
    ...
    try:
        if args.mode == "etl":
            ...
            splitter.run_split_process(table_name=args.table_name, begin_year=args.begin_year, end_year=args.end_year)
