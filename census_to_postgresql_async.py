...
class AsyncCensusDataETL:
    ...
    def connect_to_database(self):
        """Establish connection to PostgreSQL database with fallback to local"""
        # Try configured database first
        try:
            # Get credentials from AWS Secrets Manager or use local config
            ...
            logger.info("Attempting to connect to local database as fallback...")
            
            # Fallback to local database
            try:
                local_db_creds = self.config['local_database']
                
                # Create local connection string
                local_connection_string = (
                    f"postgresql://{local_db_creds['username']}:{local_db_creds['password']}"
                    f"@{local_db_creds['host']}:{local_db_creds['port']}/{local_db_creds['database']}"
                )
                ...
                logger.info("Successfully connected to local database as fallback")
                
            except Exception as local_e:
                logger.error(f"Failed to connect to local database: {local_e}")
                raise Exception(f"Could not connect to any database. Configured database error: {e}. Local database error: {local_e}")
    ...
    async def _make_census_request(self, request: CensusRequest) -> Optional[pd.DataFrame]:
        ...
        async with self.semaphore:
            try:
                ...
                if census_data is not None and not census_data.empty:
                    logger.info(f"Successfully pulled {len(census_data)} records for year {request.year}")
                    return census_data
                else:
                ...
                logger.error(f"Failed to make Census request for year {request.year}: {e}")
                raise
    ...
    async def api_iterator_async(self, begin_year: int, end_year: int) -> pd.DataFrame:
        ...
        try:
            ...
            years_list = list(range(begin_year, end_year + 1))  # Fixed: include end_year
            year_batches = [
                years_list[i:i + batch_size]
                for i in range(0, len(years_list), batch_size)
            ]
            
            logger.info(f"Processing {len(year_batches)} batches of years: {years_list}")
            
            all_results = []
            
            for i, year_batch in enumerate(year_batches):
                logger.info(f"Processing batch {i+1}/{len(year_batches)}: years {year_batch}")
    ...
    async def merge_census_with_locale(self, census_data: pd.DataFrame, locale_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            census_enhanced = census_data.merge(
                locale_data[['zip code', 'state', 'city']], 
                on='zip code', 
                how='left'
            )
            ...
            logger.info(f"Records with state: {records_with_state} ({records_with_state/total_records*100:.1f}%)")
            logger.info(f"Records with city: {records_with_city} ({records_with_city/total_records*100:.1f}%)")
            
            # Show sample of merged data
            logger.info("Sample of merged data:")
            sample_data = census_enhanced[['zip code', 'state', 'city', 'year', 'Total Pop Estimate']].head()
        ...
        try:
            ...
            with ThreadPoolExecutor() as executor:
                for i in range(0, len(locale_data_db), batch_size):
                    ...
                    logger.info(f"Inserted locale batch {i//batch_size + 1}: {len(batch)} records")
    ...
    async def save_backup_files_async(self, census_data: pd.DataFrame, locale_data: pd.DataFrame):
            ...
            with ThreadPoolExecutor() as executor:
                await asyncio.gather(
                    loop.run_in_executor(
                        executor,
                        census_data.to_csv,
                        'census_raw_async.csv'
                    ),
                    loop.run_in_executor(
                        executor,
                        locale_data.to_csv,
                        'zip_to_statecity_async.csv'
                    )
                )
    ...
    async def run_etl_async(self, begin_year: int = 2021, end_year: int = 2024):
        ...
        try:
            ...
            try:
                ...
                census_data_enhanced = await self.merge_census_with_locale(census_data, locale_data)
...
async def main():
    ...
    try:
        ...
        # Run ETL process with a smaller year range for testing
        await etl.run_etl_async(begin_year=2021, end_year=2024)
